...
- **Name entities with the device prefix** via `name`/`unique_id` properties
  returning `get_device_name() + " Entity"` and `get_serial_number() + "_suffix"`.
  This repo does NOT use `has_entity_name`. See PR #634.
- **Do not add new API calls for diagnostics.** Sensors/site should read values
  the coordinator already holds. See PR #634.
